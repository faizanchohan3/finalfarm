                ...
                {summaryLoading ? (
                  ...
                  return (
                    <tr key={f.id}
                      ...
                      className={`cursor-pointer transition-colors ${isSelected ? "bg-green-50 border-l-4 border-purple-600" : "hover:bg-slate-50"}`}>
                      ...
                      <td className="px-3 py-2">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${bal>0?"bg-red-100 text-red-700":bal<0?"bg-green-100 text-purple-700":"bg-yellow-100 text-gray-500"}`}>
                          {bal>0?"Payable":bal<0?"Advance":"Settled"}
                        </span>
                      </td>
                      <td className="px-3 py-2 text-purple-600"><ArrowRight className="w-3.5 h-3.5" /></td>
                    </tr>
                  )
                })}
          ...
          <Card className="print:hidden">
            <CardContent className="p-4">
              <div className="flex flex-wrap gap-3 items-end">
                ...
                <Button onClick={loadLedger} disabled={loading} className="bg-purple-700 hover:bg-purple-800">
                        ...
                        {ledger.entries?.map((entry: any, i: number) => (
                          <tr key={i} className={entry.type === "PAYMENT" ? "bg-green-50/40" : ""}>
                            ...
                            <td className="px-4 py-3">
                              <span className={`text-xs font-semibold px-2 py-0.5 rounded ${entry.type==="PAYMENT"?"bg-green-100 text-purple-700":entry.type==="INCOME"?"bg-blue-100 text-blue-700":entry.type==="SALE"?"bg-purple-100 text-purple-700":"bg-yellow-100 text-yellow-600"}`}>
                                {entry.type}
                              </span>
                            </td>
