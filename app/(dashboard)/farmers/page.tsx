            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading && !farmers.length ? (
                  ...
                  <>
                    <tr key={f.id}
                      className={`border-b border-slate-200 hover:bg-slate-50 cursor-pointer ${expanded[f.id] ? "bg-green-50/30" : ""}`}
                              ...
                              <div className="space-y-2">
                                {selectedPayments.size > 0 && (
                                  <div className="flex items-center gap-2 bg-blue-50 p-2 rounded border border-blue-200">
                                    ...
                                    <button onClick={() => showDeleteConfirmModal(f.id)} className="ml-auto px-2 py-1 bg-red-600 text-slate-900 text-xs rounded hover:bg-red-700">
                                      Delete Selected
                                    </button>
                                  </div>
                                )}
                                <table className="w-full text-xs border border-slate-200 rounded">
                                  ...
                                  <tbody>
                                    ...
                                    <tr className="bg-slate-50 border-t-2 border-yellow-300">
            ...
            <div>
              ...
              {photoPreview && (
                <div className="mb-2 relative">
                  ...
                  <button onClick={() => { setPhotoPreview(""); setForm({ ...form, picture: "" }); }} className="absolute -top-2 -right-2 bg-red-500 text-slate-900 rounded-full w-6 h-6 flex items-center justify-center text-xs">✕</button>
                </div>
              )}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setPayForm({ ...payForm, paymentType: "PAY" })}
                    className={`px-3 py-2.5 rounded-lg border text-sm font-medium transition-colors ${payForm.paymentType === "PAY" ? "bg-purple-700 text-slate-900 border-purple-700" : "bg-slate-50 text-gray-600 border-yellow-300 hover:border-green-300"}`}>
                    Paid to Farmer
                  ...
                  <button type="button" onClick={() => setPayForm({ ...payForm, paymentType: "RECEIVE" })}
                    className={`px-3 py-2.5 rounded-lg border text-sm font-medium transition-colors ${payForm.paymentType === "RECEIVE" ? "bg-blue-600 text-slate-900 border-blue-600" : "bg-slate-50 text-gray-600 border-yellow-300 hover:border-blue-300"}`}>
                    Received
      ...
      <Dialog open={showDetailModal} onOpenChange={setShowDetailModal}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {farmerDetail && selectedFarmer && (
            <div className="space-y-6">
              {/* Farmer Info */}
              <div className="grid grid-cols-2 gap-4 pb-4 border-b border-yellow-300">
                <div>
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {farmerDetail.purchases.map((p: any) => (
                      <div key={p.id} className="bg-slate-50 rounded p-3 text-sm border border-yellow-300">
                        <div className="flex justify-between items-start">
                          ...
                          <div className="text-right">
                            ...
                            <p className={`text-xs ${p.status === "PAID" ? "text-purple-600" : p.status === "PARTIAL" ? "text-yellow-600" : "text-gray-600"}`}>
                              {p.status}
                            </p>
                          </div>
                        </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {farmerDetail.sales.map((s: any) => (
                      <div key={s.id} className="bg-slate-50 rounded p-3 text-sm border border-yellow-300">
                        <div className="flex justify-between items-start">
                          <div>
